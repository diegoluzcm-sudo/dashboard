...
def render_tv_mode(screen, seconds, inicio, fim, contrato, meta_valor, percentual_meta, recebido, falta_meta, falta_super, funil, sdr_rank, closer_rank, daily_filtered):
    ...
    if screen == 0:
        ...
        st.markdown("<div class='section-kicker'>03 · Pré-vendas</div>", unsafe_allow_html=True)
        ...
        st.dataframe(sdr_rank[["Nome", "Vendas", "Realizadas", "No-show", "Taxa Conversão"]].sort_values("Vendas", ascending=False), hide_index=True, use_container_width=True, height=330)
        st.markdown("</div>", unsafe_allow_html=True)

    elif screen == 3:
        st.markdown("<div class='section-kicker'>04 · Closers</div>", unsafe_allow_html=True)
        st.markdown("<div class='panel'><div class='panel-title'>Ranking financeiro por Closer</div><div class='panel-caption'>Fonte oficial: aba Dashboard da planilha de ciclo.</div>", unsafe_allow_html=True)
        ...
        st.dataframe(closer_display, hide_index=True, use_container_width=True, height=330)
        st.markdown("</div>", unsafe_allow_html=True)

    else:
        st.markdown("<div class='section-kicker'>05 · Acompanhamento operacional</div>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='panel' style='margin-top:1rem'><div class='panel-title'>Ofertas pendentes / Risco Zero</div><div class='panel-caption'>Os valores cadastrados manualmente permanecem separados da meta até confirmação do pagamento na planilha oficial.</div><div style='margin-top:1rem;color:#8793a8'>Consulte a aba operacional no dashboard normal para editar as ofertas pendentes.</div></div>", unsafe_allow_html=True)
